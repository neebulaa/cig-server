...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">All Certifications</h1>
    <p class="mt-1">This page is for managing all certifications</p>
    <a href="/certifications/create"
                            ...
                            <tr>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <div class="flex items-center gap-4">
                                        <img src="{{ $certification->public_image }}"
                                            alt="image-{{ $certification->title }}"
                                            class="bg-gray-100 inline-block object-cover object-center rounded w-12 h-10">
                                        <p
                                ...
                                <td class="py-3 px-5 border-b border-blue-gray-50 max-w-[600px]">
                                    <p class="block antialiased font-sans text-sm font-medium text-blue-gray-600">
                                        {{ substr($certification->description, 0, 100) }}
                                        {{ strlen($certification->description) > 100 ? '...' : '' }}</p>
                                </td>
                                <td
                                    ...
                                    <div class="flex gap-4 justify-center items-center">
                                        <a href="/certifications/{{ $certification->slug }}" class="block">
                                        ...
                                        <a href="/certifications/edit/{{ $certification->slug }}" class="block">
                                        ...
                                        <button data-modal-target="popup-modal-{{ $certification->slug }}"
                                            data-modal-toggle="popup-modal-{{ $certification->slug }}" type="button">
                                        ...
                                        <div id="popup-modal-{{ $certification->slug }}" tabindex="-1"
                                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                            <div class="relative p-4 w-full max-w-md max-h-full">
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    <form method="post"
                                                        action="/certifications/{{ $certification->slug }}">
                                                        @csrf
                                                        @method('delete')
                                                        <button type="button"
                                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                            data-modal-hide="popup-modal-{{ $certification->slug }}">
    ...
    @else
        <h3 class="text-lg text-blue-gray-900 font-bold mt-4">Currently there is no certification
            {{ request('search') ? 'with keyword ' . request('search') : '' }}</h3>
        <p>Please add some data</p>
    @endif

    @if ($certifications->count() && !($certifications->onFirstPage() && $certifications->onLastPage()))
        <div class="mt-4">
            {{ $certifications->links('partials.paginator') }}
        </div>
    @endif
@endsection
