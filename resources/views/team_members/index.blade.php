...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">All Team Members</h1>
    <p class="mt-1">This page is for managing all team members</p>
    <a href="/team-members/create"
                            ...
                            <tr>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <div class="flex items-center gap-4">
                                        <img src="{{ $team_member->public_profile_image }}"
                                            alt="image-{{ $team_member->name }}"
                                            class="bg-gray-100 inline-block relative object-cover object-center rounded-full p-1 w-12 h-12 shrink-0">
                                        <p
                                ...
                                <td class="py-3 px-5 border-b border-blue-gray-50 max-w-[600px]">
                                    <p class="block antialiased font-sans text-sm font-medium text-blue-gray-600">
                                        {{ substr($team_member->bio, 0, 100) }}
                                        {{ strlen($team_member->bio) > 100 ? '...' : '' }}</p>
                                </td>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <p
                                        class="block antialiased font-sans text-sm font-medium text-blue-gray-600 min-w-[150px]">
                                ...
                                <td
                                    ...
                                    <div class="flex gap-4 justify-center items-center">
                                        <a href="/team-members/{{ $team_member->slug }}" class="block">
                                        ...
                                        <a href="/team-members/edit/{{ $team_member->slug }}" class="block">
                                        ...
                                        <button data-modal-target="popup-modal-{{ $team_member->slug }}"
                                            data-modal-toggle="popup-modal-{{ $team_member->slug }}" type="button">
                                        ...
                                        <div id="popup-modal-{{ $team_member->slug }}" tabindex="-1"
                                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                            <div class="relative p-4 w-full max-w-md max-h-full">
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    <form method="post" action="/team-members/{{ $team_member->slug }}">
                                                        @csrf
                                                        @method('delete')
                                                        <button type="button"
                                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                            data-modal-hide="popup-modal-{{ $team_member->slug }}">
                                                        ...
                                                        <div class="p-4 md:p-5 text-center">
                                                            ...
                                                            <h3
                                                                ...
                                                                this team member?</h3>
                                                            <button data-modal-hide="popup-modal-{{ $team_member->slug }}"
                                                                type="submit"
    ...
    @else
        <h3 class="text-lg text-blue-gray-900 font-bold mt-4">Currently there is no team member
            {{ request('search') ? 'with keyword ' . request('search') : '' }}</h3>
        <p>Please add some data</p>
    @endif

    @if ($team_members->count() && !($team_members->onFirstPage() && $team_members->onLastPage()))
        <div class="mt-4">
            {{ $team_members->links('partials.paginator') }}
        </div>
    @endif
@endsection
