...
@section('content')
    ...
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">Create Region</h1>
    <p class="mt-1">This page is for creating a region</p>
    <a href="/regions"
        ...
        to index</a>
    <hr class="mt-4 mb-4">
    <form class="p-4 border bg-white rounded-lg" method="post" action="/regions" enctype="multipart/form-data">
        @csrf
        ...
        <div class="input-box mt-4">
            ...
            <select id="type" name="type"
                ...
                <option value="province" class="capitalize" {{ old('type', '') == 'province' ? 'selected' : '' }}>Province
                </option>
                <option value="harbor" class="capitalize" {{ old('type', '') == 'harbor' ? 'selected' : '' }}>Harbor
                </option>
            </select>
            @error('type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
        ...
        <div class="input-box mt-4 {{ old('type', 'province') == 'province' ? 'block' : 'hidden' }}"
        ...
        <div class="input-box mt-4">
            <label class="block mb-2 text-sm font-sans font-medium text-gray-900 dark:text-white" for="image">Set
                Pinpoint</label>
            <input type="hidden" value="{{ old('pos-x', '') }}" id="pos-x" name="pos-x">
            <input type="hidden" value="{{ old('pos-y', '') }}" id="pos-y" name="pos-y">
            ...
            <div id="map"
                ...
                <img src="{{ asset('images/map-pin.png') }}" alt="pin map" id="pinpoint-draggable" draggable="false"
                    style="display: {{ old('pos-x') || old('pos-y') ? 'block' : 'none' }}" />
                <img src="{{ asset('images/map-white.png') }}" alt="white map" id="map-img" />
            </div>
            @if (old('pos-x') || old('pos-y'))
                <script>
                    let posXCoordinate = parseFloat({{ old('pos-x') }});
                    let posYCoordinate = parseFloat({{ old('pos-y') }});
