...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">All Regions</h1>
    <p class="mt-1">This page is for managing all regions</p>
    ...
    @if ($regions->count())
        ...
        <p class="mb-2 text-blue-gray-600 text-sm">Regions found: {{ $total_items }}</p>
                    ...
                    <tbody>
                        @foreach ($regions as $region)
                            <tr>
                                ...
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <p
                                        class="block antialiased font-sans text-sm font-medium text-blue-gray-600 capitalize">
                                        {{ $region->type }}
                                    </p>
                                </td>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                ...
                                <td class="py-3 px-5 border-b border-blue-gray-50 max-w-[600px]">
                                    <p class="block antialiased font-sans text-sm font-medium text-blue-gray-600">
                                        {{ substr($region->description, 0, 100) }}
                                        {{ strlen($region->description) > 100 ? '...' : '' }}</p>
                                </td>
                                <td
                                    ...
                                    <div class="flex gap-4 justify-center items-center">
                                        <a href="/regions/{{ $region->slug }}" class="block">
                                        ...
                                        <a href="/regions/edit/{{ $region->slug }}" class="block">
                                        ...
                                        <button data-modal-target="popup-modal-{{ $region->slug }}"
                                            data-modal-toggle="popup-modal-{{ $region->slug }}" type="button">
                                        ...
                                        <div id="popup-modal-{{ $region->slug }}" tabindex="-1"
                                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                            <div class="relative p-4 w-full max-w-md max-h-full">
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    <form method="post" action="/regions/{{ $region->slug }}">
                                                        @csrf
                                                        @method('delete')
                                                        <button type="button"
                                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                            data-modal-hide="popup-modal-{{ $region->slug }}">
                                                        ...
                                                        <div class="p-4 md:p-5 text-center">
                                                            ...
                                                            <h3
                                                                ...
                                                                this region?</h3>
                                                            <button data-modal-hide="popup-modal-{{ $region->slug }}"
                                                                type="submit"
    ...
    @else
        <h3 class="text-lg text-blue-gray-900 font-bold mt-4">Currently there is no region
            {{ request('search') ? 'with keyword ' . request('search') : '' }}</h3>
        <p>Please add some data</p>
    @endif

    @if ($regions->count() && !($regions->onFirstPage() && $regions->onLastPage()))
        <div class="mt-4">
            {{ $regions->links('partials.paginator') }}
        </div>
    @endif
@endsection
