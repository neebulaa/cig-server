...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">Edit social</h1>
    <p class="mt-1">This page is for Editing a social</p>
    <a href="/socials"
        ...
        to index</a>
    <hr class="mt-4 mb-4">
    <form class="p-4 border bg-white rounded-lg" method="post" action="/socials/{{ $social->id }}"
            ...
            <select id="company_id" name="company_id"
                ...
                @foreach ($companies as $company)
                    <option value="{{ $company->id }}"
                        {{ old('company_id', $social->company->id) == $company->id ? 'selected' : '' }}>
                        {{ $company->name }}</option>
                @endforeach
            </select>
            @error('company_id')
            ...
            <select id="type" name="type"
                ...
                @foreach ($social_medias as $social_media)
                    <option value="{{ $social_media }}" class="capitalize"
                        {{ old('type', $social->type) == $social_media ? 'selected' : '' }}>{{ $social_media }}</option>
                @endforeach
            </select>
            @error('type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
