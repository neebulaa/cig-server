...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">All Social Medias</h1>
    <p class="mt-1">This page is for managing all social medias</p>
                            ...
                            <tr>
                                ...
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <div class="flex items-center gap-2">
                                        <img src="{{ $social->public_icon }}" alt="image-{{ $social->type }}"
                                            class="bg-gray-100 p-1 inline-block relative object-contain object-center rounded w-8 h-8">
                                        <p
                                            class="capitalize block antialiased font-sans text-sm font-medium text-blue-gray-600 min-w-[150px]">
                                            {{ $social->type }}
                                        </p>
                                    </div>
                                </td>
                                ...
                                <td
                                    ...
                                    <div class="flex gap-4 justify-center">
                                        <a href="/socials/edit/{{ $social->id }}" class="block">
                                        ...
                                        <button data-modal-target="popup-modal-{{ $social->id }}"
                                            data-modal-toggle="popup-modal-{{ $social->id }}" type="button">
                                        ...
                                        <div id="popup-modal-{{ $social->id }}" tabindex="-1"
                                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                            <div class="relative p-4 w-full max-w-md max-h-full">
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    <form method="post" action="/socials/{{ $social->id }}">
                                                        @csrf
                                                        @method('delete')
                                                        <button type="button"
                                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                            data-modal-hide="popup-modal-{{ $social->id }}">
                                                        ...
                                                        <div class="p-4 md:p-5 text-center">
                                                            ...
                                                            <h3
                                                                ...
                                                                this social?</h3>
                                                            <button data-modal-hide="popup-modal-{{ $social->id }}"
                                                                type="submit"
    ...
    @else
        <h3 class="text-lg text-blue-gray-900 font-bold mt-4">Currently there is no social
            {{ request('search') ? 'with keyword ' . request('search') : '' }}</h3>
        <p>Please add some data</p>
    @endif

    @if ($socials->count() && !($socials->onFirstPage() && $socials->onLastPage()))
        <div class="mt-4">
            {{ $socials->links('partials.paginator') }}
        </div>
    @endif
@endsection
