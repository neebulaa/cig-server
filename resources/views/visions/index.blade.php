...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">All Visions</h1>
    <p class="mt-1">This page is for managing all visions</p>
    <a href="/visions/create"
                            ...
                            <tr>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <div class="flex items-center gap-4">
                                        <img src="{{ $vision->public_image }}" alt="image-{{ $vision->name }}"
                                ...
                                <td class="py-3 px-5 border-b border-blue-gray-50 max-w-[600px]">
                                    <p class="block antialiased font-sans text-sm font-medium text-blue-gray-600">
                                        {{ substr($vision->description, 0, 100) }}
                                        {{ strlen($vision->description) > 100 ? '...' : '' }}</p>
                                </td>
                                <td
                                    ...
                                    <div class="flex gap-4 justify-center items-center">
                                        <a href="/visions/{{ $vision->slug }}" class="block">
                                        ...
                                        <a href="/visions/edit/{{ $vision->slug }}" class="block">
                                        ...
                                        <button data-modal-target="popup-modal-{{ $vision->slug }}"
                                            data-modal-toggle="popup-modal-{{ $vision->slug }}" type="button">
                                        ...
                                        <div id="popup-modal-{{ $vision->slug }}" tabindex="-1"
                                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                            <div class="relative p-4 w-full max-w-md max-h-full">
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    <form method="post" action="/visions/{{ $vision->slug }}">
                                                        @csrf
                                                        @method('delete')
                                                        <button type="button"
                                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                            data-modal-hide="popup-modal-{{ $vision->slug }}">
                                                        ...
                                                        <div class="p-4 md:p-5 text-center">
                                                            ...
                                                            <h3
                                                                ...
                                                                this vision?</h3>
                                                            <button data-modal-hide="popup-modal-{{ $vision->slug }}"
                                                                type="submit"
    ...
    @else
        <h3 class="text-lg text-blue-gray-900 font-bold mt-4">Currently there is no vision
            {{ request('search') ? 'with keyword ' . request('search') : '' }}</h3>
        <p>Please add some data</p>
    @endif

    @if ($visions->count() && !($visions->onFirstPage() && $visions->onLastPage()))
        <div class="mt-4">
            {{ $visions->links('partials.paginator') }}
        </div>
    @endif
@endsection
