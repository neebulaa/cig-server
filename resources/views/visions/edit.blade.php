...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">Edit Vision</h1>
    <p class="mt-1">This page is for editing a vision</p>
    <a href="/visions"
        ...
        to index</a>
    <hr class="mt-4 mb-4">
    <form class="p-4 border bg-white rounded-lg" method="post" action="/visions/{{ $vision->slug }}"
        ...
        <div class="input-box mt-4">
            <label class="block mb-2 text-sm font-sans font-medium text-gray-900 dark:text-white" for="image">Upload
                Image</label>
            <img alt="image-preview" src="{{ $vision->public_image }}"
                class="bg-gray-100 image-preview w-72 aspect-video p-1 max-w-full rounded-md object-cover mt-4 mb-4">
            <input
    ...
    <script>
        ...
        title.addEventListener('change', function() {
            fetch(`/visions/create_slug?title=${title.value}`).then(res => res.json()).then(data => slug.value =
                data
                .slug);
        });
    </script>
