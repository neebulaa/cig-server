...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">All Comodities</h1>
    <p class="mt-1">This page is for managing all comodities</p>
    <a href="/comodities/create"
    ...
    @if ($comodities->count())
        <p class="mb-2 text-blue-gray-600 text-sm">Comodities found: {{ $total_items }}</p>
                            ...
                            <tr>
                                ...
                                <td
                                    ...
                                    <div class="flex gap-4 justify-center">
                                        <a href="/comodities/edit/{{ $comodity->slug }}" class="block">
                                        ...
                                        <button data-modal-target="popup-modal-{{ $comodity->slug }}"
                                            data-modal-toggle="popup-modal-{{ $comodity->slug }}" type="button">
                                        ...
                                        <div id="popup-modal-{{ $comodity->slug }}" tabindex="-1"
                                            class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                            <div class="relative p-4 w-full max-w-md max-h-full">
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    <form method="post" action="/comodities/{{ $comodity->slug }}">
                                                        @csrf
                                                        @method('delete')
                                                        <button type="button"
                                                            class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                                            data-modal-hide="popup-modal-{{ $comodity->slug }}">
                                                        ...
                                                        <div class="p-4 md:p-5 text-center">
                                                            ...
                                                            <h3
                                                                ...
                                                                this comodity?</h3>
                                                            <button data-modal-hide="popup-modal-{{ $comodity->slug }}"
                                                                type="submit"
    ...
    @else
        <h3 class="text-lg text-blue-gray-900 font-bold mt-4">Currently there is no comodity
            {{ request('search') ? 'with keyword ' . request('search') : '' }}</h3>
        <p>Please add some data</p>
    @endif

    @if ($comodities->count() && !($comodities->onFirstPage() && $comodities->onLastPage()))
        <div class="mt-4">
            {{ $comodities->links('partials.paginator') }}
        </div>
    @endif
@endsection
