...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">All Companies</h1>
    <p class="mt-1">This page is for managing all companies</p>
                            ...
                            <tr>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <div class="flex items-center gap-4">
                                        <img src="{{ $company->public_logo }}" alt="image-{{ $company->name }}"
                                ...
                                <td class="py-3 px-5 border-b border-blue-gray-50 max-w-[600px]">
                                    <p class="block antialiased font-sans text-sm font-medium text-blue-gray-600">
                                        {{ substr($company->about, 0, 100) }}
                                        {{ strlen($company->about) > 100 ? '...' : '' }}</p>
                                </td>
                                <td class="py-3 px-5 border-b border-blue-gray-50">
                                    <p
                                ...
                                <td
                                    ...
                                    <div class="flex gap-4 justify-center items-center">
                                        ...
                                        <a href="/companies/edit/{{ $company->slug }}" class="block">
                                        ...
                                        <button data-modal-target="popup-modal-{{ $company->slug }}"
                                            data-modal-toggle="popup-modal-{{ $company->slug }}" type="button">
                                        ...
                                        <div id="popup-modal-{{ $company->slug }}" tabindex="-1"
                                                ...
                                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                    <form method="post" action="/companies/{{ $company->slug }}">
                                                        ...
                                                        <button type="button"
                                                            ...
                                                            data-modal-hide="popup-modal-{{ $company->slug }}">
                                                        ...
                                                        <div class="p-4 md:p-5 text-center">
                                                            ...
                                                            <h3
                                                                ...
                                                                this company?</h3>
                                                            <button data-modal-hide="popup-modal-{{ $company->slug }}"
                                                                type="submit"
    ...
    @else
        <h3 class="text-lg text-blue-gray-900 font-bold mt-4">Currently there is no company
            {{ request('search') ? 'with keyword ' . request('search') : '' }}</h3>
        <p>Please add some data</p>
    @endif

    @if ($companies->count() && !($companies->onFirstPage() && $companies->onLastPage()))
        <div class="mt-4">
            {{ $companies->links('partials.paginator') }}
        </div>
    @endif
@endsection
