...
@section('content')
    <h1 class="text-2xl text-blue-gray-900 font-bold mt-4">Create Company</h1>
    <p class="mt-1">This page is for creating a company</p>
    <a href="/companies"
        ...
        to index</a>
    <hr class="mt-4 mb-4">
    <form class="p-4 border bg-white rounded-lg" method="post" action="/companies" enctype="multipart/form-data">
        @csrf
        ...
        <div class="input-box mt-4">
            ...
            <textarea cols="30" rows="2" name="address" id="address"
                class="resize-none border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('address') border-red-500 @enderror">{{ old('address', '') }}</textarea>
            @error('address')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="input-box mt-4">
            ...
            <textarea cols="30" rows="4" name="iframe_src" id="iframe_src"
                class="resize-none border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-3 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('iframe_src') border-red-500 @enderror">{{ old('iframe_src', '') }}</textarea>
            @error('iframe_src')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="input-box mt-4">
            ...
            <img alt="image-preview"
                class="image-preview w-[150px] h-[150px] max-w-full rounded-md object-contain mt-4 mb-4 hidden">
            <input
